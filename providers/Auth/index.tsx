...
export const AuthProvider: React.FC<{ children: React.ReactNode; api?: 'rest' | 'gql' }> = ({
  ...
  const create = useCallback<Create>(
    async args => {
      if (api === 'rest') {
        const user = await rest(`${apiUrl}/api/users`, args)
        setUser(user)
        return user
      }
  ...
  const login = useCallback<Login>(
    async args => {
      if (api === 'rest') {
        const user = await rest(`${apiUrl}/api/users/login`, args)
        setUser(user)
        return user
      }
  ...
  const logout = useCallback<Logout>(async () => {
    if (api === 'rest') {
      await rest(`${apiUrl}/api/users/logout`)
      setUser(null)
      return
    }
  ...
  const resetPassword = useCallback<ResetPassword>(
    async args => {
      if (api === 'rest') {
        const user = await rest(`${apiUrl}/api/users/reset-password`, args)
        setUser(user)
        return user
      }
